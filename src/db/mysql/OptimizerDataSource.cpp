...
public:
    ...
    void getPairBWLimits(const Pair &pair, std::map<std::string, double> &link_limits) {
        ...
        soci::use(proj_id, "proj_id"));

        for (auto j = limits.begin(); j != limits.end(); ++j) {
            auto plink_id = j->get<std::string>("plink_id");
            auto limit = j->get<double>(limit, 0.0);
    ...
    // FIXME: not use this at this moment
        ...
        std::map<std::string, TransferredStat> &thr_map)
    {
		...
		time_t totalSeconds = now - windowStart;

        // get and set the limits - initalize the transferred bytes
        std::map<std::string, double> link_limits;
        getPairBWLimits(pair, link_limits);

        for (auto it = link_limits.begin(); it != link_limits.end(); it++) {
        ...
        " WHERE t1.source_se = :sourceSe"
        "      AND t1.dest_se = :destSe"
        "      AND t3.vo_name = :voName"
